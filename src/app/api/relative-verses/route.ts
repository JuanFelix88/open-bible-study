...
export async function GET(req: NextRequest) {
  ...
  const [word] = Params.getParamFromSearchParams("word", url.searchParams);
  ...
  const normalizedVersionAbbr = versionAbbr?.trim() || undefined;
  ...
  const { data, error } = await FnNormalizer.getFromPromise(
    BibleVersionsRepository.getRelativeVerses({
      word: normalizedWord,
      versionAbbr: normalizedVersionAbbr,
      count: safeCount,
    }),
  );
