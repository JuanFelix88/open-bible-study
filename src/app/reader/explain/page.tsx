...
export default function Explain() {
  ...
  const { data: verseAnalysis, isLoading: isLoadingVerseAnalysis } = useQuery({
    ...
    queryFn: async () => {
      ...
      const language = verseExplainResponse.headers.get("language")?.toString();
      const explainVersion = verseExplainResponse.headers
        .get("version")
        ?.toString();

      return {
        explainData: explainData as VerseAnalysis[],
        modelName,
  ...
  return (
    <div className="flex min-h-screen flex-col px-7 py-7 pb-15 bg-background text-text max-w-[750px] w-screen">
      ...
      <div
        ...
        dir={verseAnalysis?.language === Language.HE ? "rtl" : "ltr"}
      >
