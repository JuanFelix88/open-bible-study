...
async function throwGenevaError(response: Response) {
  let errorMessage = "";
  ...
  throw new Error(errorMessage || `Geneva Study Bible returned ${response.status}`);
}

function getGenevaErrorMessage(
  error: unknown,
  reference: string,
) {
  const message = error instanceof Error ? error.message : "";

  if (message.toLowerCase().includes("note not found for this verse")) {
    return `A Geneva Study Bible não possui nota específica para ${reference}. O Blue Letter Bible exibe apenas os versículos que têm notas; quando o verso não aparece na fonte, não há comentário Geneva para carregar.`;
  }

  if (message.toLowerCase().includes("timeout") || message.toLowerCase().includes("aborted")) {
    return "A consulta à Geneva Study Bible excedeu o limite de 5 segundos. Tente novamente em instantes.";
  }

  return `Não foi possível carregar as notas da Geneva Study Bible para ${reference}.`;
}

export default function ReaderGenevaStudyBibleComments() {
  ...
  const {
    data: commentsMarkdown,
    isLoading: isLoadingComments,
    isFetching: isFetchingComments,
    error: commentsError,
  } = useQuery({
    queryKey: ["geneva-study-bible-comments", version, bookAbbr, chapterNumber, verseNumber],
    ...
    queryFn: async () => {
      const commentsResponse = await fetch(
        `/comments/geneva/${encodeURIComponent(bookAbbr)}/${chapterNumber}/${verseNumber}?version=${encodeURIComponent(version ?? "")}`,
      );

      if (!commentsResponse.ok) {
        await throwGenevaError(commentsResponse);
      }

      return commentsResponse.text();
    },
  });
  ...
  const referenceText = `${chapter?.book.name || bookAbbr || "..."} ${chapterText}:${verseNumber || "..."}`;
  const commentsErrorMessage = getGenevaErrorMessage(
    commentsError,
    referenceText,
  );
    ...
    <div className="flex min-h-screen flex-col px-7 py-7 pb-15 bg-background text-text max-w-[750px] w-screen">
      ...
      {isLoading && (
        <div className="flex flex-col gap-3 mt-5 animate-show-from-bottom-slow">
          <div className="flex items-center gap-2 text-sm text-text/50">
            ...
            />
            <span>
              Fetching Geneva Study Bible, translating, and formatting...
            </span>
          </div>
      ...
      {commentsError && !isLoading && (
        <div className="mt-5 rounded-md border border-danger/40 bg-danger/10 p-4 text-sm text-text/80 animate-show-from-bottom-slow">
          <strong className="block text-danger mb-1">
        ...
        <article className="mt-5 animate-show-from-bottom-slow">
          <Markdown
            components={{
              ...
              h2: ({ children }) => {
                const firstSectionText = String(children);
                const isFirstSection =
                  firstSectionText.includes("Geneva") ||
                  firstSectionText.toLowerCase().includes("texto");
